...
class WalletUserFlow(SequentialTaskSet):
    ...
    def _make_initial_deposit(self):
        ...
        payload = {
            "payeeCvu": self.cvu,  # Usar CVU como entero
        ...
        with self.client.post(
            ...
            if response.ok:
                ...
                response.failure(f"Initial deposit failed: {response.text}")
                print(f"❌ Error en depósito inicial: {response.text}")
    ...
    def step_4_make_deposit(self):
        ...
        with self.client.post(
            ...
            if response.ok:
                ...
                print(f"❌ Error en depósito: {response.text}")
    ...
    def step_5_make_withdrawal(self):
        ...
        payload = {
            "payerCvu": self.cvu,  # Usar CVU como entero
        ...
        with self.client.post(
            ...
            if response.ok:
                ...
                response.failure(f"Withdrawal failed: {response.text}")
                print(f"❌ Error en retiro: {response.text}")
    ...
    def step_6_make_p2p_transfer(self):
        ...
        payload = {
            "payerCvu": self.cvu,  # Usar CVU como entero
            "payeeCvu": destination_cvu,  # Usar CVU como entero
            ...
            try:
                if response.status_code in [200, 201]:
                    ...
                    if response_data.get("status") == "COMPLETED":
                        ...
                        print(f"⚠️ Estado inesperado en transferencia: {response_data}")
                elif response.status_code == 404:
                    # CVU destino no encontrado - esto es esperado
                    response.success()
                    print(f"ℹ️ CVU destino no encontrado - Error esperado")
                elif response.status_code == 400:
                    error_msg = response.json().get('message', response.text)
                    response.failure(f"Error de validación en transferencia: {error_msg}")
                    print(f"❌ Error de validación: {error_msg}")
                else:
                    response.failure(f"Transferencia falló ({response.status_code}): {response.text}")
                    print(f"❌ Error en transferencia: {response.text}")
            except Exception as e:
                response.failure(f"Error inesperado en transferencia: {str(e)}")
    ...
    def step_7_get_transaction_history(self):
        ...
        with self.client.get(
            ...
            if response.status_code == 200:
                ...
                response.failure(f"Consulta de historial falló: {response.text}")
